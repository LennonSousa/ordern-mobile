...
import { StoreDeliveryGroups } from '../../components/StoreDeliveryGroups';
...
export default function Shipment() {
    ...
    const [storeShipments, setStoreShipments] = useState<StoreShipments[]>([]);
    const [storeDeliveryGroups, setStoreDeliveryGroups] = useState<StoreDeliveryGroups[]>([]);
    const [selectedDeliveryGroup, setSelectedDeliveryGroup] = useState<StoreDeliveryGroups>();
    ...
    useEffect(() => {
        if (customer) {
            ...
            api.get(`customer/${customer.id}/store/shipments`).then(res => {
                setStoreShipments(res.data);

                api.get(`customer/${customer.id}/store/delivery-groups`).then(response => {
                    setStoreDeliveryGroups(response.data);
                    ...
                    .catch(err => {
                        console.log('Error to get store delivery groups');
                        console.log(err);
    ...
    function handleDeliveryGroup(deliveryGroup: StoreDeliveryGroups) {
    ...
    return (
        <>
            <ScrollView style={globalStyles.container}>
                {
                    storeShipments && storeShipments.length > 0 && storeShipments.find(shipment => { return shipment.code === 'pickup' }) &&
                ...
                {
                    storeShipments && storeShipments.length > 0 && storeShipments.find(shipment => { return shipment.code === 'neighborhoods' }) &&
                    <>
                        <View style={globalStyles.row}>
                            <View style={globalStyles.column}>
                                <View style={globalStyles.menuRow}>
                        ...
                        {
                            storeDeliveryGroups && storeDeliveryGroups.map((deliveryGroup, index) => {
